...
# Mark Tutorials as the active section on every generated tutorial page.
header_html = header_html.replace(
...
# Remove inherited homepage canonical tag to avoid duplicates
header_html = header_html.replace('<link rel="canonical" href="https://theomicshub.com/" />\n', '')
header_html = header_html.replace('<link rel="canonical" href="https://theomicshub.com/" />', '')
...
for index, tut in enumerate(tutorials):
    ...
    # Get the global index of the current tutorial
    current_idx = next((i for i, t in enumerate(tutorials) if t['id'] == tutorial_id), 0)

    if current_idx > 0:
        prev_tut = tutorials[current_idx - 1]
        is_diff_cat = prev_tut['category'] != cat_str
        label = "Previous Stage" if is_diff_cat else "Previous Lesson"
        prev_html = f'''
        <a href="{prev_tut['id']}.html" class="group relative flex flex-col justify-center p-6 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden text-left">
            ...
            <div class="relative z-10 flex items-center gap-4">
                ...
                <div>
                    <span class="block text-xs font-bold tracking-widest text-gray-400 uppercase mb-1">{label}</span>
    ...
    if current_idx < len(tutorials) - 1:
        next_tut = tutorials[current_idx + 1]
        is_diff_cat = next_tut['category'] != cat_str
        label = "Next Stage" if is_diff_cat else "Next Lesson"
        next_html = f'''
